         ...
         if (context.args.size() == 1) {
             ...
             if (it == cmds.end()) {
                 std::string suggestion =
                     utils::find_closest_command(cmds, input);
                 if (!suggestion.empty()) {
             ...
             dpp::embed embed =
                 dpp::embed()
                     .set_title(string(dpp::unicode_emoji::books) + " help: `" +
                                cmd.name + "`")
                     .set_color(dpp::colors::blue)
                     .add_field(
                         string(dpp::unicode_emoji::pushpin) + " Description",
                         cmd.description.empty() ? "No description available"
                                                 : cmd.description)
                     .add_field(string(dpp::unicode_emoji::paperclip) +
                                    string(" Alias") +
                                    (cmd.aliases.size() > 1 ? "es" : ""),
                                cmd.aliases.empty()
                                    ? "No aliases available"
                                    : utils::join(cmd.aliases, ", "))
                     .add_field(string(dpp::unicode_emoji::memo) + " Usage",
                                utils::format_usage(cmd))
                     .add_field(
                         string(dpp::unicode_emoji::stopwatch) + " Cooldown",
                         cmd.cooldown == 0
                             ? "No cooldown"
                             : to_string(cmd.cooldown) + " seconds")
                     .set_footer(
                         dpp::embed_footer()
                             .set_text("Requested by " +
                                       context.message.author.username)
                             .set_icon(
                                 context.message.author.get_avatar_url()));

             context.event.reply(
                 dpp::message(context.message.channel_id, embed));
         } else {
             dpp::embed embed =
                 dpp::embed()
             ...
             for (const auto &[name, cmd] : cmds) {
                 ...
                 std::string description = cmd.description.empty()
                                               ? "No description"
                                               : cmd.description;
